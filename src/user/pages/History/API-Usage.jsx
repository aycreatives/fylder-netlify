...
export default function VerificationsHistoryTable() {
  ...
  const filteredTransactions = apiData.filter((transaction) => {
    // Filter for API-Usage transactions only
    if (transaction.TransactionType !== "API-Usage") return false;
    ...
    // Text search filter for API Usage specific fields
    const passesSearchFilter =
      transaction.apiKey?.toLowerCase().includes(searchStr) ||
                ...
                {paginatedData.map((transaction, index) => (
                  ...
                  >
                    ...
                    <td className="w-[clamp(120px,20vw,160px)] px-2 py-2 whitespace-nowrap">
                      <span className="text-[clamp(0.65rem,1vw,0.75rem)] font-medium">
                    ...
                    <td className="w-[clamp(80px,15vw,112px)] px-2 py-2 whitespace-nowrap">
                      <span
